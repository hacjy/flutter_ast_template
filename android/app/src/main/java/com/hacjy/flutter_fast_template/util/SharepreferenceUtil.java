...
public class SharepreferenceUtil {
    ...
    public static <T> T jsonToClass(Context context, String fileName, String nodeName) {
        String json = getSharePreString(context, fileName, nodeName, "");
        return json != null && !json.equals("") ? (T) jsonToClass(json) : null;
    }

    public static <T> T jsonToClass(String string) {
        try {
            byte[] mBytes = Base64.decode(string.getBytes(), 0);
    ...
    public static <T> void saveClass(Context context, String fileName, String nodeName, T object) {
        try {
    ...
    public static <T> List<T> jsonToList(Context context, String fileName, String nodeName) {
        String json = getSharePreString(context, fileName, nodeName, "");
        return json != null && !json.equals("") ? (List<T>) jsonToList(json) : null;
    }
